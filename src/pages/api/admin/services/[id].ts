...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  ...
  await db
    .prepare(
      `UPDATE services SET name=?, tagline=?, description=?, image_url=?, sort_order=?, is_active=?, is_featured=?, updated_at=datetime('now')
       WHERE id=?`
    )
    .bind(
      b.name, b.tagline || '', b.description || '', b.image_url || '',
